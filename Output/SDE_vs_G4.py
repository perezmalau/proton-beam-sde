...
def plot_bragg_peak(g4_mat, *other_mats, how='proj', names=None, ref_name='Geant4',
    ...
    #colours = ['darkblue', 'darkorange', 'forestgreen', 'deeppink', 'gold', 'darkred']
    colours = plt.cm.tab10(np.arange(len(others)))
    for i, (mat, name, c) in enumerate(zip(others, names, colours)):
        if how == 'proj':
        ...
        ax.plot(x, dose1d, label=name, color=c)
        ax_diff.plot(x, compute_percentage_difference(g4_dose1d, dose1d), color=c, label=f'{name} vs {ref_name}')
    ...
    # Difference
    ax_diff.set_ylabel("Diff vs EMZ [%]")
...
def plot_lateral_profiles(g4_mat, sde_mat,
    ...
    for i, cut in enumerate(cuts):
        ax.plot(z, g4_mat[cut, g4_mat.shape[1] // 2, :], color=colors[i])
        ax.scatter(z, sde_mat[cut, sde_mat.shape[1] // 2, :], color=colors[i], marker='^')

        # Custom legend
    depth_lines = [Line2D([0], [0], color=c, lw=2, label=f'{d} cm') for c, d in zip(colors, depths)]
    model_lines = [
...
# Compares 1D plots for two proton energies
# Let sde_dose1 be the sde matrix at energy1, g4_dose1 the geant4 matrix at energy1 and so on
def compare_bragg_peaks(sde_dose1, g4_dose1, sde_dose2, g4_dose2, energy1, energy2):
